...
class Kegiatan_model extends CI_Model {
	...
	var $table = 'm_kegiatan';
	var $column_order = array(null, 'nama_kegiatan','kode_unor',null); //set column field database for datatable orderable
	var $column_search = array('nama_kegiatan','kode_unor'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	{    
		...
		$id_kegiatan = ($exist) ? $id :  getID('id_kegiatan', $this->table);
		
		$prop = array('id_kegiatan'=> $id_kegiatan);				
		...
		if($exist)			
		{	
			$this->db_ba->where('id_kegiatan' , $id_kegiatan);
	...
	public function get_by_id($id, $field = '*')
	{
		...
		$this->db_ba->from($this->table);
		$this->db_ba->where('id_kegiatan',$id);
		$query = $this->db_ba->get();
