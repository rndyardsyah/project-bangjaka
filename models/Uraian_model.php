...
class Uraian_model extends CI_Model {
	...
	var $table = 't_uraian_pekerjaan';
	var $column_order = array('uraian', 'volume', 'satuan',' keterangan', null); //set column field database for datatable orderable
	var $column_search = array('uraian', 'volume', 'satuan',' keterangan'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	{		
		...
		$this->db_ba->where("id_hasil_pekerjaan", $_POST['id_hasil_pekerjaan']);
	...
	{    
		$where = array('id_uraian_pekerjaan' => $id);
		$this->db_ba
		->where($where)
		->from($this->table);
		$exist = $this->db_ba->get()->row();	
				
		$id_uraian_pekerjaan = ($exist) ? $id :  getID('id_uraian_pekerjaan', $this->table);
		
		$prop = array('id_uraian_pekerjaan'=> $id_uraian_pekerjaan);				
	...
	public function get_by_id($id, $field = '*')
	{
		...
		$this->db_ba->from($this->table);
		$this->db_ba->where('id_uraian_pekerjaan',$id);
		$query = $this->db_ba->get();
