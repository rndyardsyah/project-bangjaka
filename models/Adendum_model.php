...
class Adendum_model extends CI_Model {
	...
	var $table = 't_adendum';
	var $column_order = array(null, 'a.no_adendum','a.tgl_adendum','a.biaya_adendum','a.waktu_pelaksanaan_adendum',null); //set column field database for datatable orderable
	var $column_search = array('a.no_adendum','a.tgl_adendum','a.biaya_adendum','a.waktu_pelaksanaan_adendum'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	{		
		...
		$this->db_ba->where("a.id_pembayaran", @$_POST['id_pembayaran']);		
	...
	{    
		$where = array('id_adendum' => $id);
		$this->db_ba
		->where($where)
		->from($this->table);
		$exist = $this->db_ba->get()->row();	
				
		$id_adendum = ($exist) ? $id :  getID('id_adendum', $this->table);
		
		$prop = array('id_adendum'=> $id_adendum);				
		...
		if($exist)			
		{	
			$this->db_ba->where('id_adendum' , $id_adendum);
	...
	public function get_by_id($id, $field = '*')
	{
		...
		$this->db_ba->from($this->table);
		$this->db_ba->where('id_adendum',$id);
		$query = $this->db_ba->get();
	...
	function get_wherev2($where='', $field='*', $order=false){
		$this->db_ba->select($field);
		
		if($order){
			$this->db_ba->order_by('tgl_adendum', 'ASC');
		}
