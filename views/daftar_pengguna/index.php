...
$(document).ready(function() {	
	...
	if(<?php echo $this->session->userdata('id_akses'); ?> == 1){
		...
		$(".tombl").html('<button class="btn btn-primary btn-xs dropdown-toggle" data-toggle="dropdown" type="button"><i class="fa fa-cogs fa-fw"></i></button><ul class="dropdown-menu slidedown"><li><a href="javascript:void(0)" onclick="add_sso_dinas()"><i class="glyphicon glyphicon-plus-sign"></i> Tambah User Pegawai <sup>sso</sup> </a></li></ul>');
	}
...
function kembali()
{
	...
	$('#btnModal_cancel').text('Cancel'); // Set Title to Bootstrap modal title
	$('#page-wrapper-form').html(''); //reset / menghilangkan form menu dalam div 
	$('#datagreed_rincian').html(''); //reset
	$('#formModal [name="id"]').val(''); //reset
...
function add()
{
	...
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', function(data, status) 
		{			
			...
			$(".page-header").html('Daftar Pengguna <small>Tambah Data</small><div class="btn-group pull-right tombl"></div>');
...
{
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form_sso');?>', function(data, status) 
		{			
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Daftar Pengguna <small>Tambah Data</small><div class="btn-group pull-right tombl"></div>');
...
function add_sso_dinas()
{
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form_sso_dinas');?>', function(data, status) 
		{			
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Daftar Pengguna <small>Tambah Data</small><div class="btn-group pull-right tombl"></div>');
...
function edit_sso(id)
{
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form_sso');?>', {id_user:id}, function(data, status) 
		{
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Daftar Pengguna <small>Ubah Data</small><div class="btn-group pull-right tombl"></div>');
			$(".tombl").html('<a href="javascript:void(0)" onclick="kembali()" class="btn btn-primary"><i class="glyphicon glyphicon-backward"></i> kembali</a>');
			$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');				
			// var returnedData = JSON.parse(data);
...
function edit_no_sso(id)
{
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', {id_user:id}, function(data, status) 
		{
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Daftar Pengguna <small>Ubah Data</small><div class="btn-group pull-right tombl"></div>');
...
function tampil_rincian_detail(data){	

	if(data){		
		$('.input_rincian_kegiatan').show();
		$('[name="dpa_skpd"]').val('');
		$('[name="kode_rek"]').val('');
		$('#table-rincian').html('');
		$('#formModal [name="id"]').val('');
		$('#datagreed_rincian').html(''); //reset
		$('[name="pagu"]').val(0);
	...
	$("#loading-overlay").show();
	$('#datagreed_rincian').load('<?=base_url('ba/'.$class_name.'/ajax_insert');?>', {data:data}, function() 
		{					
			$("#loading-overlay").hide();
		}
	);
...
function getListRincianKegiatan()
{	
	var data = $('#id_kegiatan').val();
	// alert(data);
	
	if(data){
		var cek_id = $('#formModal [name="id"]').val();
		
		if(cek_id){
			$('#formModal [name="id"]').val(data);	
			$('.form-group').removeClass('has-error'); // clear error class
			$('#modal_form').modal('show'); // show bootstrap modal
			$('.modal-title').text('Rincian Kegiatan'); // Set Title to Bootstrap modal title
		...
		obj = JSON.parse(data);
		var kode_rek = obj.kode_rek;
		$('[name="dpa_skpd"]').val(kode_rek);
		$('[name="kode_rek"]').val(kode_rek+'5.2');
	}	
}
