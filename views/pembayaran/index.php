...
<div class="row">
	<div class="col-lg-12">
		...
		<h3 class="page-header">Data Pengajuan Pembayaran
					...
					<div class="panel-body">
						<table id="table" class="table table-hover table-striped " cellspacing="0" width="100%" >
							<thead>
								<tr>
									<th width="3%">Aksi</th>
									<th width="75%">Data Permohonan Pembayaran</th>
									<th width="22%">Tanggal Permohonan</th>
								</tr>
							</thead>
							<tbody>
							</tbody>
...
$(document).ready(function() {	
	...
	<?php if($this->session->userdata('id_akses') == 10){ ?>
		$(".tombl").html('');
	<?php }else{ ?>
	...
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', function(data, status) 
		{
			...
			$(".page-header").html('Data Pengajuan Pembayaran <small>Tambah Data</small><div class="btn-group pull-right tombl"></div>');
	...
	$('#page-wrapper-form').load('<?=base_url('ba/hasil_pekerjaan/ajax_form');?>', {id_pencairan:id}, function(data, status) 
		{
			...
			$(".page-header").html('Data Pengajuan Pembayaran <small>Hasil Pekerjaan -> Tambah Data</small><div class="btn-group pull-right tombl"></div>');
			$(".tombl").html('<a href="javascript:void(0)" onclick="kembali()" class="btn btn-primary"><i class="glyphicon glyphicon-backward"></i> kembali</a>');
			$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');	
			// var returnedData = JSON.parse(data);
		}
...
{
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', {id_pembayaran:id}, function(data, status) 
		{
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			...
			// $('#id_pencairan').attr("disabled", "disabled");
			// $('[name="id_pencairan"]').val(21);
			$('.selectpicker').attr('data-live-search', 'true');
			$('.selectpicker').selectpicker('refresh');
			$('#id_pencairan').prop('disabled','disabled');
		}
	);
}
...
{
	save_method = 'set_date';
	
	$('[name="id"]').val(id);
	$('#formModal')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
	$('#modal_form').modal('show'); // show bootstrap modal
	$('.modal-title').text('Ganti Tanggal Pembayaran'); // Set Title to Bootstrap modal title
	$('#btnModal_proses').text('Proses'); // Set Title to Bootstrap modal title
	$('#form-tambahan').html('<div class="form-group"><label class="control-label col-md-3">Tanggal </label><div class="col-md-9"><div class="dateContainer"><div class="input-group date datetimePicker" id="tgl_permohonan_pembayaran"><span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span><input class="form-control" name="tgl_permohonan_pembayaran" id="tgl_permohonan_pembayaran" value="<?php echo date('d-m-Y'); ?>" placeholder="DD-MM-YYYY" data-date-format="DD-MM-YYYY" type="text"><span class="help-block"></span></div></div></div></div>'); // Add Teks
	$('#kalimat_tampil').html(''); // Add Teks
	
	
	$('.datetimePicker #tgl_permohonan_pembayaran').datetimepicker(
	{
		pickTime: false 
	}
	);
...
function cetak(id){
	$("#loading-overlay").show();
	$.ajax({
		url : "<?php echo base_url('ba/'.$class_name.'/download_pdf')?>",
		type: "POST",
		...
		success: function(data)
		{
			...
			{			
				window.open("<?=base_url(); ?>"+data.url_file, '_blank');
			}
		},
...
function delete_proses()
{
	...
	$("#loading-overlay").show();
	var url;
	if(save_method == 'set_date'){
		url = "<?php echo base_url('ba/'.$class_name.'/ajax_set_date')?>/";
	}else if(save_method == 'delete_adendum'){
		url = "<?php echo base_url('ba/adendum/ajax_delete')?>/";
	}else{
...
function set_last_year(id){	
	$("#loading-overlay").show();
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form_last_year');?>', {id_pembayaran:id}, function(data, status) 
		{
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Data Pengajuan Pembayaran <small>Last Year</small><div class="btn-group pull-right tombl"></div>');
			$(".tombl").html('<a href="javascript:void(0)" onclick="kembali()" class="btn btn-primary"><i class="glyphicon glyphicon-backward"></i> kembali</a>');
			$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');	
			// var returnedData = JSON.parse(data);
		}
