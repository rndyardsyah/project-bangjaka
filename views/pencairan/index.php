...
<div class="row">
	<div class="col-lg-12">
		...
		<h3 class="page-header">Data Pengajuan Pencairan
	...
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', function(data, status) 
		{
			...
			$(".page-header").html('Data Pengajuan Pencairan <small>Tambah Data</small><div class="btn-group pull-right tombl"></div>');
	...
	$('#page-wrapper-form').load('<?=base_url('ba/hasil_pekerjaan/ajax_form');?>', {id_pencairan:id}, function(data, status) 
		{
			...
			$(".page-header").html('Data Pengajuan Pencairan <small>Hasil Pekerjaan -> Tambah Data</small><div class="btn-group pull-right tombl"></div>');
			$(".tombl").html('<a href="javascript:void(0)" onclick="kembali()" class="btn btn-primary"><i class="glyphicon glyphicon-backward"></i> kembali</a>');
			$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');	
			// var returnedData = JSON.parse(data);
		}
	...
	$('#page-wrapper-form').load('<?=base_url('ba/'.$class_name.'/ajax_form');?>', {id_penyedia:id}, function(data, status) 
		{
			$("#loading-overlay").hide();
			$("#page-wrapper-datatable").hide();
			$(".page-header").html('Data Pengajuan Pencairan <small>Ubah Data</small><div class="btn-group pull-right tombl"></div>');
			$(".tombl").html('<a href="javascript:void(0)" onclick="kembali()" class="btn btn-primary"><i class="glyphicon glyphicon-backward"></i> kembali</a>');
			$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');				
			// var returnedData = JSON.parse(data);
		}
