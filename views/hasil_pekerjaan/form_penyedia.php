...
function save(kondisi)
{
    ...
    var url;
	url = "<?php echo base_url('ba/'.$class_name.'/ajax_save_penyedia')?>";
	...
	formData.append('data_pegawai', '<?=@$data_pegawai; ?>');
    $.ajax({
        url : url,
        type: "POST",
        data: formData,
...
function proses_hasil_pekerjaan(){	
	...
	$("#btnModal_proses").attr("onclick","save(2)");
...
function save_proses()
{
    ...
    var url;
	url = "<?php echo base_url('ba/hasil_pekerjaan/ajax_proses')?>";
    // ajax adding data to database
	$("#loading-overlay").show();
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            {	
				...
				$('html, body').animate({scrollTop:$('#wrapper').offset().top - 50}, 'slow');			
				$(".page-header").html('Data Pengajuan Hasil Pekerjaan');
            ...
            else
            {
                if(data.inputerror){
					...
					{
						$('[name="'+data.inputerror[i]+'"]').parent().parent().addClass('has-error'); //select parent twice to select div form-group class and add has-error class
						if(data.inputerror[i] == 'keterangan'){
							$('[name="'+data.inputerror[i]+'"]').next().next().text(data.error_string[i]); //select span help-block class set text error string						
						}else{
							$('[name="'+data.inputerror[i]+'"]').next().text(data.error_string[i]); //select span help-block class set text error string
						}
		...
		<div class="panel panel-default">					
			<div class="panel-body">
							...
							<div class="panel panel-primary">
								...
								<div id="collapse1" class="panel-collapse in">
								  <div class="panel-body">						
									...
									<div class="form-group">
										<label class="control-label col-md-3" id="label-photo">Upload Hasil Pekerjaan <br><i><font color="red">*Format .pdf, maks 10Mb</font></i> </label>
										<div class="col-md-9">
						...
						<div class="modal-footer">
							<button type="button" id="btnProsesHasilPekerjaan" onclick="proses_hasil_pekerjaan()" class="btn btn-success">Proses Hasil Pekerjaan</button>
